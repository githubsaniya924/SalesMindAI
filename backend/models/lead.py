...
class Lead(db.Model):
    ...
    # relationships
    outreaches = db.relationship("OutreachLog", backref="lead")
    enrichments = db.relationship("LeadEnrichment", backref="lead")
    ...
    def create(cls, **kwargs):
        """Creates and saves a new Lead record."""
        try:
            # Map kwargs to the model fields. Note: We use the names from _format_apollo_lead
            new_lead = cls(
                ...
                # You might need to add logic for name and job_title if they don't match exactly
            )
            db.session.add(new_lead)
            db.session.commit()
            return new_lead
        except Exception as e:
            # Rollback on error to ensure database integrity
            db.session.rollback()
            raise e
